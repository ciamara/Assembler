.386
rozkazy SEGMENT use16
ASSUME cs:rozkazy

snake PROC

    ; przechowanie rejestrow
    push ax
    push bx
    push cx
    push di
    push si
    push es

    mov ax, 0A000H ; adres pamięci ekranmov cs:adres_ogona, bxu dla trybu 13H
    mov es, ax

    ; czyszczenie poprzedniego piksela weza
    mov bx, cs:adres_piksela
    mov al, 0
    mov es:[bx], al
    mov es:[bx+1], al
    mov es:[bx+2], al
    mov es:[bx+320], al
    mov es:[bx+321], al
    mov es:[bx+322], al
    mov es:[bx+640], al
    mov es:[bx+641], al
    mov es:[bx+642], al

    mov di, cs:adres_myszy
    mov al, 0
    mov es:[di], al
    mov es:[di+1], al
    mov es:[di+2], al
    mov es:[di+320], al
    mov es:[di+321], al
    mov es:[di+322], al
    mov es:[di+640], al
    mov es:[di+641], al
    mov es:[di+642], al


    ;ruch myszy
    cmp cs:kierunek_myszy, 0
    je ruch_dol_mysz
    cmp cs:kierunek_myszy, 1
    je ruch_gora_mysz
    cmp cs:kierunek_myszy, 2
    je ruch_lewo_mysz
    cmp cs:kierunek_myszy, 3
    je ruch_prawo_mysz

    ruch_dol_mysz:
    add di, 320
    cmp di, 320*200
    jb next
    sub di, 320*200 
    jmp next

    ruch_gora_mysz:
    sub di, 320
    cmp di, 0
    jge next
    ;add bx, 320*200 ;PROBLEM
    jmp next

    ruch_lewo_mysz:
    sub di, 3
    jmp next

    ruch_prawo_mysz:
    add di, 3

next:

    ; zmiana kierunku w zależności od przycisków strzałek
    cmp cs:kierunek, 0
    je ruch_dol
    cmp cs:kierunek, 1
    je ruch_gora
    cmp cs:kierunek, 2
    je ruch_lewo
    cmp cs:kierunek, 3
    je ruch_prawo

    ruch_dol:
    add bx, 320
    cmp bx, 320*200
    jb narysuj
    sub bx, 320*200 
    jmp narysuj

    ruch_gora:
    sub bx, 320
    cmp bx, 0
    jge narysuj
    ;add bx, 320*200 ;PROBLEM
    jmp narysuj

    ruch_lewo:
    sub bx, 3
    jmp narysuj

    ruch_prawo:
    add bx, 3

narysuj:

    ; narysowanie nowego piksela weza 3x3 w kolorze białym
    mov al, 15
    mov es:[bx], al
    mov es:[bx+1], al
    mov es:[bx+2], al
    mov es:[bx+320], al
    mov es:[bx+321], al
    mov es:[bx+322], al
    mov es:[bx+640], al
    mov es:[bx+641], al
    mov es:[bx+642], al

    cmp cs:red_pixel1_active, 1
    jne brak_pixel1

    ;narysowanie nowego piksela myszy 3x3 w kolorze czerwonym
    mov al, 4
    mov es:[di], al
    mov es:[di+1], al
    mov es:[di+2], al
    mov es:[di+320], al
    mov es:[di+321], al
    mov es:[di+322], al
    mov es:[di+640], al
    mov es:[di+641], al
    mov es:[di+642], al

    ; Sprawdzenie, czy biała kropka dotyka czerwonej
    cmp bx, di
    je usun_czerwony1

brak_pixel1:

    mov cs:adres_piksela, bx
    mov cs:adres_myszy, di
    jmp koniec_ruchu

usun_czerwony1:

    mov al, 0               
    mov es:[di], al
    mov es:[di+1], al
    mov es:[di+2], al
    mov es:[di+320], al
    mov es:[di+321], al
    mov es:[di+322], al
    mov es:[di+640], al
    mov es:[di+641], al
    mov es:[di+642], al
    
    mov cs:red_pixel1_active, 0

koniec_ruchu:

    ; Aktualizacja pozycji piksela w cs:adres_piksela
    mov cs:adres_piksela, bx
    mov cs:adres_myszy, di

    ; Narysowanie nowego piksela po kolizji (kontynuacja ruchu)
    mov al, 15
    mov es:[bx], al
    mov es:[bx+1], al
    mov es:[bx+2], al
    mov es:[bx+320], al
    mov es:[bx+321], al
    mov es:[bx+322], al
    mov es:[bx+640], al
    mov es:[bx+641], al
    mov es:[bx+642], al

    ; odtworzenie rejestrow
    pop es
    pop si
    pop di
    pop cx
    pop bx
    pop ax

    ; skok do oryginalnego podprogramu obsługi przerwania zegarowego
    jmp dword PTR cs:wektor8

    ; zmienne procedury
    kolor db 15 ; biały kolor
    adres_piksela dw (320 * 100) + 160 ; bieżący adres piksela
    adres_myszy dw (320*50) +100 ;biezacy adres myszy
    kierunek db 0 ; 0 = dół, 1 = góra, 2 = lewo, 3 = prawo
    kierunek_myszy db 0 ; 0 = dół, 1 = góra, 2 = lewo, 3 = prawo
    wektor8 dd ?  
    red_pixel1_active db 1

snake ENDP

zacznij:

    mov ah, 0
    mov al, 13H ; tryb graficzny
    int 10H
    mov bx, 0
    mov es, bx
    mov eax, es:[32]
    mov cs:wektor8, eax

    mov ax, SEG snake
    mov bx, OFFSET snake

    cli
    mov es:[32], bx
    mov es:[32+2], ax
    sti

czekaj:

    mov ah, 1
    int 16h
    jz czekaj
    mov ah, 0
    int 16h
    cmp al, 1Bh ; kod ASCII dla klawisza ESC
    je koniec
    cmp ah, 48h ; strzałka w górę
    je zmien_gora
    cmp ah, 50h ; strzałka w dół
    je zmien_dol
    cmp ah, 4Bh ; strzałka w lewo
    je zmien_lewo
    cmp ah, 4Dh ; strzałka w prawo
    je zmien_prawo
    jmp czekaj

zmien_gora:
    mov cs:kierunek, 1
    jmp czekaj

zmien_dol:
    mov cs:kierunek, 0
    jmp czekaj

zmien_lewo:
    mov cs:kierunek, 2
    jmp czekaj

zmien_prawo:
    mov cs:kierunek, 3
    jmp czekaj

koniec:
    mov ah, 0
    mov al, 3H
    int 10H

    mov eax, cs:wektor8
    mov es:[32], eax

    mov ax, 4C00H
    int 21H

rozkazy ENDS

stosik SEGMENT stack

    db 256 dup (?)

stosik ENDS

END zacznij